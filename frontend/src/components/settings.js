var Settings = {
  ...
  show: function () {
    ...
    if (window.Moonfin && window.Moonfin.Plugin)
      window.Moonfin.Plugin._overlayHistoryDepth++;
  ...
  saveSetting: function (name, value) {
    ...
    var safeValue =
      name.toLowerCase().indexOf("apikey") !== -1 ||
      name.toLowerCase().indexOf("token") !== -1
        ? "***"
        : value;
    console.log(
      '[Moonfin] Setting saved to profile "' + profileName + '":',
      name,
      "=",
      safeValue,
    );
  ...
  createToggleCard: function (id, title, description, checked) {
    return (
      '<div class="moonfin-toggle-card">' +
      '<label class="moonfin-toggle-label">' +
      '<input type="checkbox" id="moonfin-' +
      id +
      '" name="' +
      id +
      '"' +
      ...
      '<div class="moonfin-toggle-title">' +
      title +
      "</div>" +
      (description
        ? '<div class="moonfin-toggle-desc">' + description + "</div>"
        : "") +
      "</div>" +
      "</label>" +
      "</div>"
  ...
  createRangeCard: function (
    ...
    return (
      ...
      '<div class="moonfin-toggle-title">' +
      title +
      ' <span class="moonfin-range-value" data-for="' +
      id +
      '">' +
      currentValue +
      (suffix || "") +
  ...
  createSection: function (icon, title, contentHtml, openByDefault) {
    return (
      '<details class="moonfin-panel-section"' +
      (openByDefault ? " open" : "") +
      ">" +
      '<summary class="moonfin-panel-summary">' +
      (icon ? icon + " " : "") +
      title +
      "</summary>" +
  ...
  createDialog: function () {
    ...
    var uiContent =
      this.createToggleCard(
        "navbarEnabled",
        "Navigation Bar",
        "Show the custom navigation bar with quick access buttons",
        settings.navbarEnabled,
      ) +
      this.createSelectCard(
        "navbarPosition",
        "Navbar Position",
        "Show the navigation bar at the top or as a left sidebar",
        [
          { value: "top", label: "Top" },
      ...
      this.createToggleCard(
        "mediaBarEnabled",
        "Media Bar",
        "Show the featured media carousel on the home page",
        settings.mediaBarEnabled,
      ) +
      this.createToggleCard(
        "detailsPageEnabled",
        "Details Page",
        "Use the custom Moonfin details page instead of the default Jellyfin one",
        settings.detailsPageEnabled,
      );
    ...
    var mediaBarContent =
      this.createSelectCard(
        "mediaBarSourceType",
        "Content Source",
        "Where to pull media bar items from",
        [
      ...
      "</div>" +
      this.createToggleCard(
        "mediaBarShuffleItems",
        "Shuffle Items",
        "Randomize the order of items from selected collections",
        settings.mediaBarShuffleItems !== false,
      ) +
      "</div>" +
      this.createSelectCard(
        "mediaBarItemCount",
      ...
      this.createToggleCard(
        ...
        "Trailer Preview",
        "Automatically play muted trailer previews in the media bar background",
        settings.mediaBarTrailerPreview,
      );
    ...
    var overlayContent =
      this.createSelectCard(
        ...
        "Overlay Color",
        "Color of the gradient overlay on media bar items",
        colorOptions,
        settings.mediaBarOverlayColor,
      ) +
      '<div class="moonfin-color-preview" id="moonfin-color-preview" style="background:' +
      Storage.getColorHex(settings.mediaBarOverlayColor) +
      '"></div>' +
      this.createRangeCard(
        "mediaBarOverlayOpacity",
        "Overlay Opacity",
        "Transparency of the gradient overlay",
        0,
        100,
        5,
        settings.mediaBarOverlayOpacity,
        "%",
      );
    ...
    var toolbarContent =
      this.createToggleCard(
        "showShuffleButton",
        "Shuffle Button",
        "Show random content button in the toolbar",
        settings.showShuffleButton,
      ) +
      this.createSelectCard(
        "shuffleContentType",
        "Shuffle Content Type",
        "What type of content to shuffle",
        [
          { value: "both", label: "Movies & TV Shows" },
          { value: "movies", label: "Movies Only" },
          { value: "tv", label: "TV Shows Only" },
      ...
      this.createToggleCard(
        "showGenresButton",
        "Genres Button",
        "Show genres dropdown in the toolbar",
        settings.showGenresButton,
      ) +
      this.createToggleCard(
        "showFavoritesButton",
        "Favorites Button",
        "Show favorites button in the toolbar",
        settings.showFavoritesButton,
      ) +
      this.createToggleCard(
        "showCastButton",
        "Cast Button",
        "Show Chromecast button in the toolbar",
        settings.showCastButton,
      ) +
      this.createToggleCard(
        "showSyncPlayButton",
        "SyncPlay Button",
        "Show SyncPlay button in the toolbar",
        settings.showSyncPlayButton,
      ) +
      this.createToggleCard(
        "showLibrariesInToolbar",
        "Library Shortcuts",
        "Show library quick links in the toolbar",
        settings.showLibrariesInToolbar,
      );
    ...
    var displayContent =
      this.createToggleCard(
        "showClock",
        "Clock",
        "Show a clock in the navigation bar",
        settings.showClock,
      ) +
      this.createToggleCard(
        "use24HourClock",
      ...
      this.createSelectCard(
        "seasonalSurprise",
        "Seasonal Effect",
        "Add a seasonal visual effect to the interface",
        seasonalOptions,
    ...
    var selectedSources = settings.mdblistRatingSources || [
      "imdb",
      "tmdb",
      "tomatoes",
      "metacritic",
    ...
    var serverUrl =
      (window.ApiClient && window.ApiClient.serverAddress
    ...
    for (var oi = 0; oi < selectedSources.length; oi++) {
      for (var oj = 0; oj < mdblistSources.length; oj++) {
        if (mdblistSources[oj].key === selectedSources[oi]) {
          orderedSources.push({
            key: mdblistSources[oj].key,
            label: mdblistSources[oj].label,
            enabled: true,
          });
          break;
        }
    ...
    for (var uk = 0; uk < mdblistSources.length; uk++) {
      if (selectedSources.indexOf(mdblistSources[uk].key) === -1) {
        orderedSources.push({
          key: mdblistSources[uk].key,
          label: mdblistSources[uk].label,
          enabled: false,
        });
      }
    }
    ...
    for (var si = 0; si < orderedSources.length; si++) {
      ...
      var iconUrl =
        serverUrl +
        "/Moonfin/Assets/" +
        (sourceIconFiles[src.key] || "imdb.svg");
      sourceItems +=
        '<div class="moonfin-sortable-item' +
        (src.enabled ? " moonfin-sortable-item-active" : "") +
        '" draggable="true" data-source="' +
        src.key +
        '">' +
        ...
        "</span>" +
        '<img class="moonfin-sortable-icon" src="' +
        iconUrl +
        '" alt="' +
        src.label +
        '">' +
        '<span class="moonfin-sortable-label">' +
        src.label +
        "</span>" +
        '<button type="button" class="moonfin-sortable-toggle" title="' +
        (src.enabled ? "Disable" : "Enable") +
        '">' +
        '<svg viewBox="0 0 24 24" width="16" height="16"><path fill="currentColor" d="' +
        (src.enabled
    ...
    var tmdbContent =
      this.createToggleCard(
        ...
        "Enable Episode Ratings",
        "Show TMDB ratings for individual TV episodes on the details page",
        settings.tmdbEpisodeRatingsEnabled,
      ) +
      '<div class="moonfin-tmdb-config" style="' +
      (settings.tmdbEpisodeRatingsEnabled ? "" : "display:none") +
      '">' +
      ...
      '<input type="password" id="moonfin-tmdbApiKey" class="moonfin-panel-input" placeholder="' +
      (Storage.syncState.tmdbAvailable
        ? "Using server key (optional override)"
        : "Enter your TMDB API key or v4 token") +
      '" value="' +
      (settings.tmdbApiKey || "") +
      '">' +
    ...
    var mdblistContent =
      this.createToggleCard(
        ...
        "Show ratings from MDBList (IMDb, Rotten Tomatoes, Metacritic, etc.) on media bar and item details",
      ...
      '<input type="password" id="moonfin-mdblistApiKey" class="moonfin-panel-input" placeholder="' +
      (Storage.syncState.mdblistAvailable
        ? "Using server key (optional override)"
        : "Enter your mdblist.com API key") +
      '" value="' +
      ...
      '<div class="moonfin-sortable-list" id="moonfin-sources-sortable">' +
      sourceItems +
      "</div>" +
      "</div>" +
      "</div>";
    ...
    for (var pi = 0; pi < profileNames.length; pi++) {
      ...
      profileTabsHtml +=
        '<button type="button" class="moonfin-profile-tab' +
        (isActive ? " moonfin-profile-tab-active" : "") +
        '" data-profile="' +
        pn +
        '">' +
        profileLabels[pn] +
        (isCurrent
          ? ' <span class="moonfin-profile-current-badge" title="Current device">●</span>'
          : "") +
        "</button>";
    }
    profileTabsHtml += "</div>";
    var profileInfoHtml =
      '<div class="moonfin-profile-info">' +
    ...
    this.dialog.innerHTML =
      ...
      '<input type="checkbox" id="moonfin-sync-toggle"' +
      (Storage.isSyncEnabled() ? " checked" : "") +
      ">" +
  ...
  refreshFormValues: function (profileName) {
    ...
    // Update checkboxes
    var checkboxes = this.dialog.querySelectorAll(
    ...
    for (var i = 0; i < checkboxes.length; i++) {
      ...
      if (name in resolved) {
        ...
        var isInherited =
          raw !== null && (raw[name] === undefined || raw[name] === null);
    ...
    for (var j = 0; j < selects.length; j++) {
      ...
      if (sName in resolved) {
        ...
        if (sCard && raw !== null) {
          sCard.classList.toggle(
            "moonfin-inherited",
            raw[sName] === undefined || raw[sName] === null,
          );
        }
      }
    }
    ...
    for (var k = 0; k < ranges.length; k++) {
      ...
      if (rName in resolved) {
        ...
        var valueSpan = this.dialog.querySelector(
          '.moonfin-range-value[data-for="' + rName + '"]',
        );
    ...
    if (colorPreview) {
      colorPreview.style.background = Storage.getColorHex(
        resolved.mediaBarOverlayColor,
      );
    }
    ...
    if (mdblistConfig)
      mdblistConfig.style.display = resolved.mdblistEnabled ? "" : "none";
    var tmdbConfig = this.dialog.querySelector(".moonfin-tmdb-config");
    if (tmdbConfig)
      tmdbConfig.style.display = resolved.tmdbEpisodeRatingsEnabled
        ? ""
        : "none";

    // Toggle media bar source sub-sections
    var isCollection = resolved.mediaBarSourceType === "collection";
    var libraryOpts = this.dialog.querySelector(
      ".moonfin-mediabar-library-options",
    );
    var collectionOpts = this.dialog.querySelector(
      ".moonfin-mediabar-collection-options",
    );
    ...
    if (infoText) {
      if (profileName === "global") {
        ...
        infoText.textContent =
          "Editing " +
          label +
          ' overrides. Dimmed settings are inherited from "All Devices". Changes here only affect ' +
          label +
          " devices.";
      }
    }
  ...
  loadCollectionPicker: function () {
    ...
    var picker = this.dialog
      ? this.dialog.querySelector("#moonfin-collection-picker")
      : null;
    if (!picker) return;
    ...
    API.getCollectionsAndPlaylists()
      .then(function (items) {
        if (!self.dialog) return;
        if (!items || items.length === 0) {
          picker.innerHTML =
            '<div class="moonfin-toggle-desc">No collections or playlists found on this server.</div>';
          return;
        }
        ...
        for (var i = 0; i < items.length; i++) {
          ...
          if (item.ImageTags && item.ImageTags.Primary) {
            posterUrl =
              ...
              "/Items/" +
              item.Id +
              "/Images/Primary?maxWidth=60&quality=80&tag=" +
          ...
          html +=
            ...
            ">" +
            (posterUrl
              ? '<img class="moonfin-collection-poster" src="' +
                posterUrl +
                '" alt="">'
        ...
        picker.addEventListener("change", function (e) {
          ...
          self.showToast(
            cb.checked ? "Added to media bar" : "Removed from media bar",
          );
        });
      })
      .catch(function (e) {
  ...
  updateJellyseerrSsoSection: function () {
    var self = this;
    var wrapper = this.dialog
      ? this.dialog.querySelector(".moonfin-settings-jellyseerr-wrapper")
      : null;
    ...
    return Jellyseerr.fetchConfig().then(function () {
      if (
        !Jellyseerr.config ||
        !Jellyseerr.config.enabled ||
        !Jellyseerr.config.url
      ) {
        console.log(
          "[Moonfin] Jellyseerr not configured, hiding section. Config:",
          Jellyseerr.config,
        );
      ...
      var indicator = wrapper.querySelector(
        ".moonfin-jellyseerr-sso-indicator",
      );
      ...
      var signedInGroup = wrapper.querySelector(
        ".moonfin-jellyseerr-signedIn-group",
      );
      ...
      return Jellyseerr.checkSsoStatus().then(function () {
        if (Jellyseerr.ssoStatus && Jellyseerr.ssoStatus.authenticated) {
          ...
          if (api && api._currentUser) {
            var usernameInput = wrapper.querySelector(
              "#jellyseerr-settings-username",
            );
  ...
  updateSyncStatus: function () {
    var self = this;
    var statusEl = this.dialog
      ? this.dialog.querySelector("#moonfinSyncStatus")
      : null;
  ...
  setupEventListeners: function () {
    ...
    this.dialog
      .querySelector(".moonfin-settings-close")
      .addEventListener("click", function () {
        self.hide();
      });

    this.dialog
      .querySelector(".moonfin-settings-close-btn")
      .addEventListener("click", function () {
        self.hide();
      });

    this.dialog
      .querySelector(".moonfin-settings-overlay")
      .addEventListener("click", function () {
        self.hide();
      });

    this.dialog
      .querySelector(".moonfin-settings-reset")
      .addEventListener("click", function () {
        ...
        if (activeProfile !== "global") {
          if (
            confirm(
              'Reset "' +
                activeProfile +
                '" device profile? This will remove all overrides for this device.',
            )
          ) {
    ...
    this.dialog
      .querySelector(".moonfin-settings-sync")
      .addEventListener("click", function () {
      ...
      (function (cb) {
        cb.addEventListener("change", function () {
          ...
          if (cb.name === "mdblistEnabled") {
            var configDiv = self.dialog.querySelector(
              ".moonfin-mdblist-config",
            );
            if (configDiv) {
          ...
          if (cb.name === "tmdbEpisodeRatingsEnabled") {
            var tmdbConfigDiv = self.dialog.querySelector(
              ".moonfin-tmdb-config",
            );
            if (tmdbConfigDiv) {
    ...
    for (var k = 0; k < ranges.length; k++) {
      (function (range) {
        range.addEventListener("input", function () {
          var valueSpan = self.dialog.querySelector(
            '.moonfin-range-value[data-for="' + range.name + '"]',
          );
          if (valueSpan) {
    ...
    var colorSelect = this.dialog.querySelector(
    ...
    var sourceTypeSelect = this.dialog.querySelector(
      'select[name="mediaBarSourceType"]',
    );
    if (sourceTypeSelect) {
      sourceTypeSelect.addEventListener("change", function () {
        var isCollection = sourceTypeSelect.value === "collection";
        var libraryOpts = self.dialog.querySelector(
          ".moonfin-mediabar-library-options",
        );
        var collectionOpts = self.dialog.querySelector(
          ".moonfin-mediabar-collection-options",
        );
        if (libraryOpts) libraryOpts.style.display = isCollection ? "none" : "";
        if (collectionOpts)
    ...
    var mdblistApiKeyInput = this.dialog.querySelector(
      "#moonfin-mdblistApiKey",
    );
      ...
      (function () {
        ...
        sortableList.addEventListener(
          ...
          function (e) {
            ...
            touchClone.className =
              "moonfin-sortable-item moonfin-sortable-touch-clone";
        ...
        sortableList.addEventListener(
          ...
          function (e) {
            ...
            var items = sortableList.querySelectorAll(
              ".moonfin-sortable-item:not(.moonfin-sortable-dragging)",
            );
            for (var i = 0; i < items.length; i++) {
    ...
    var loginBtn = this.dialog.querySelector(
      ".moonfin-jellyseerr-settings-login-btn",
    );
    if (loginBtn) {
    ...
    var passwordInput = this.dialog.querySelector(
      "#jellyseerr-settings-password",
    );
    if (passwordInput) {
    ...
    var authTypeBtns = this.dialog.querySelectorAll(
      ".moonfin-segmented-btn[data-auth-type]",
    );
    for (var ati = 0; ati < authTypeBtns.length; ati++) {
      authTypeBtns[ati].addEventListener("click", function () {
        var wrapper = self.dialog.querySelector(
          ".moonfin-settings-jellyseerr-wrapper",
        );
        if (!wrapper) return;
        var btns = wrapper.querySelectorAll(
          ".moonfin-segmented-btn[data-auth-type]",
        );
        for (var j = 0; j < btns.length; j++)
          btns[j].classList.remove("moonfin-segmented-btn-active");
        ...
        var usernameLabel = wrapper.querySelector(
          ".moonfin-jellyseerr-username-label",
        );
        if (desc)
        ...
        if (usernameLabel)
          usernameLabel.textContent = isLocal ? "Email" : "Username";
        var passwordField = wrapper.querySelector(
          "#jellyseerr-settings-password",
        );
        if (passwordField)
          passwordField.placeholder = isLocal
            ? ""
            : "Leave empty if no password";
      });
    }

    var logoutBtn = this.dialog.querySelector(
      ".moonfin-jellyseerr-settings-logout-btn",
    );
    if (logoutBtn) {
      logoutBtn.addEventListener("click", function () {
        if (
          confirm(
            "Sign out of Jellyseerr? You will need to sign in again to use it.",
          )
        ) {
  ...
  handleJellyseerrLogin: function () {
    ...
    var submitBtn = wrapper.querySelector(
      ".moonfin-jellyseerr-settings-login-btn",
    );
    ...
    var activeAuthBtn = wrapper.querySelector(
      ".moonfin-segmented-btn-active[data-auth-type]",
    );
    var authType = activeAuthBtn
      ? activeAuthBtn.getAttribute("data-auth-type")
      : "jellyfin";
    var isLocalAuth = authType === "local";
    if (!usernameVal) {
      errorEl.textContent = isLocalAuth
    ...
    Jellyseerr.ssoLogin(usernameVal, passwordVal, authType).then(
      function (result) {
        if (result.success) {
          self.updateJellyseerrSsoSection();
