var Settings = {
    ...
    saveSetting: function (name, value) {
        ...
        var safeValue =
            name.toLowerCase().indexOf('apikey') !== -1 || name.toLowerCase().indexOf('token') !== -1 ? '***' : value;
    ...
    createDialog: function () {
        ...
        var serverUrl =
            (window.ApiClient && window.ApiClient.serverAddress ? window.ApiClient.serverAddress() : '') || '';
        var sourceIconFiles = {
            imdb: 'imdb.svg',
            tmdb: 'tmdb.svg',
            trakt: 'trakt.svg',
        ...
        for (var si = 0; si < orderedSources.length; si++) {
            ...
            sourceItems +=
                ...
                (src.enabled ? 'M9 16.17L4.83 12l-1.42 1.41L9 19 21 7l-1.41-1.41z' : 'M19 13H5v-2h14v2z') +
                '"/></svg>' +
                '</button>' +
                '</div>';
        }
        ...
        API.getUserViews()
            .then(function (views) {
                ...
                for (var j = 0; j < libraries.length; j++) {
                    ...
                    var typeBadge =
                        (lib.CollectionType || 'library').charAt(0).toUpperCase() +
    ...
    setupEventListeners: function () {
        ...
        this.dialog.querySelector('.moonfin-settings-reset').addEventListener('click', function () {
            ...
            if (activeProfile !== 'global') {
                if (
                    confirm(
                        'Reset "' + activeProfile + '" device profile? This will remove all overrides for this device.'
                    )
                ) {
        ...
        if (sortableList) {
            ...
            sortableList.addEventListener('click', function (e) {
                ...
                if (svg) {
                    svg.setAttribute(
                        'd',
                        isActive ? 'M9 16.17L4.83 12l-1.42 1.41L9 19 21 7l-1.41-1.41z' : 'M19 13H5v-2h14v2z'
                    );
                }
