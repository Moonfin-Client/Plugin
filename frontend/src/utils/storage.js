const Storage = {
  ...
  saveProfile(profileName, settings, syncToServer = true) {
    try {
      ...
      window.dispatchEvent(
        new CustomEvent("moonfin-settings-changed", { detail: resolved }),
      );

      if (
        syncToServer &&
        this.syncState.serverAvailable &&
        this.isSyncEnabled()
      ) {
  ...
  resolveSettings(profileName) {
    ...
    for (const key of allKeys) {
      ...
      if (device[key] !== undefined && device[key] !== null) {
        ...
        adminDefaults[key] !== undefined &&
        adminDefaults[key] !== null
      ) {
  ...
  async pingServer() {
    try {
      ...
      if (response.ok) {
        ...
        this.syncState.serverAvailable =
          data.installed && data.settingsSyncEnabled;
        ...
        if (data.defaultSettings) {
          this.syncState.adminDefaults = this._mapProfileFromServer(
  ...
  async saveProfileToServer(profileName, profileSettings) {
    ...
    try {
      ...
      const response = await fetch(
        `${serverUrl}/Moonfin/Settings/Profile/${profileName}`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
  ...
  _mapEnvelopeFromServer(serverData) {
    ...
    if (
      ...
      const profiles = {};
      if (serverData.global)
        profiles.global = this._mapProfileFromServer(serverData.global);
      if (serverData.desktop)
        profiles.desktop = this._mapProfileFromServer(serverData.desktop);
      if (serverData.mobile)
        profiles.mobile = this._mapProfileFromServer(serverData.mobile);
      if (serverData.tv)
        profiles.tv = this._mapProfileFromServer(serverData.tv);
      return {
        profiles: profiles,
  ...
  _mapEnvelopeToServer(profiles) {
    ...
    if (profiles.global)
      envelope.global = this._mapProfileToServer(profiles.global);
    if (profiles.desktop)
      envelope.desktop = this._mapProfileToServer(profiles.desktop);
    if (profiles.mobile)
      envelope.mobile = this._mapProfileToServer(profiles.mobile);
  ...
  async sync(forceFromServer = false) {
    console.log(
      "[Moonfin] Starting settings sync..." +
        (forceFromServer ? " (server wins)" : ""),
    );
    ...
    if (forceFromServer && serverProfiles) {
      ...
      merged = this.threeWayMergeProfiles(
        localProfiles,
        serverProfiles,
        snapshot,
      );
    ...
    try {
      ...
      window.dispatchEvent(
        new CustomEvent("moonfin-settings-changed", { detail: this.getAll() }),
      );
    } catch (e) {
