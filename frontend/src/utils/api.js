const API = {
  ...
  async getRandomItems(options = {}) {
    ...
    try {
      ...
      const baseParams = {
        ...
        hasThemeVideo: false,
        fields:
          "Overview,Genres,CommunityRating,CriticRating,OfficialRating,RunTimeTicks,ProductionYear,ProviderIds",
        imageTypeLimit: 1,
  ...
  async getGenreItems(genreName, options) {
    ...
    try {
      ...
      var params = {
        ...
        enableTotalRecordCount: true,
        fields:
          "PrimaryImageAspectRatio,ProductionYear,CommunityRating,OfficialRating,RunTimeTicks,Overview,Genres",
        imageTypeLimit: 1,
  ...
  getServerId: function () {
    ...
    if (!api) return null;
    try {
      if (api.serverInfo && typeof api.serverInfo === "function")
        return api.serverInfo().Id;
      if (api._serverInfo) return api._serverInfo.Id;
      if (api.serverId && typeof api.serverId === "function")
        return api.serverId();
    } catch (e) {}
    return null;
  },
  ...
  _initWebpackRequire: function () {
    ...
    if (!chunkArr) {
      ...
      for (var i = 0; i < keys.length; i++) {
        if (
          keys[i].indexOf("webpackChunk") === 0 &&
          Array.isArray(window[keys[i]])
        ) {
  ...
  _loadEditorModules: function () {
    ...
    if (
      window.__moonfin_editors &&
      Object.keys(window.__moonfin_editors).length >= 4
    ) {
    ...
    this._editorModulesPromise = new Promise(function (resolve) {
      ...
      if (!shortcutsId) {
        console.warn(
          "[Moonfin] Could not find shortcuts module in webpack factories",
        );
        resolve(false);
        return;
      }
      ...
      var shortcutChunkIds = apiSelf._extractChunkIds(
        req.m[shortcutsId].toString(),
      );
      ...
      apiSelf
        ...
        .then(function () {
          ...
          var icmId = apiSelf._findFactoryByHints(req, [
            "editimages",
            "editsubtitles",
            "identify",
          ]);
          if (icmId) {
            ...
            console.log(
              "[Moonfin] Loading itemContextMenu chunks:",
              icmChunkIds,
            );
  ...
  _matchEditorExports: function (mod, editors) {
    ...
    if (
      ...
      typeof mod.showFindNew === "function"
    ) {
  ...
  openItemIdentifier: function (itemId) {
    ...
    return this._loadEditorModules()
      .then(function (loaded) {
        var ed =
          window.__moonfin_editors && window.__moonfin_editors.identifier;
        if (!ed) return false;
        ed.show(itemId, serverId);
        return true;
      })
